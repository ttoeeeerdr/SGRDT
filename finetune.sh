...
# export NCCL_IB_DISABLE=1
# 通信方式
export NCCL_IB_DISABLE=1            # 桌面环境禁用InfiniBand
export NCCL_P2P_DISABLE=0           # 启用GPU直接通信

# 超时配置（跨NUMA通信需要更长时间）
export NCCL_TIMEOUT=3600            # NCCL操作超时1小时
export TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC=3600  # PyTorch心跳超时
export TORCH_NCCL_BLOCKING_WAIT=1   # 使用阻塞等待，更稳定

# 跨NUMA优化与缓冲
export NCCL_CROSS_NIC=0             # 不使用跨NIC通信
export NCCL_BUFFSIZE=67108864       # 64MB缓冲区

# 调试与网络接口
export NCCL_DEBUG=WARN              # 正常训练用WARN即可
# export NCCL_SOCKET_IFNAME=enp9s0    # 指定为你机器上实际激活的主网卡
...
export WANDB_API_KEY="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"
