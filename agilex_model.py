...
def load_vggt_model(model_path, device="cuda"):
    """懒加载 VGGT 模型，只加载一次"""
    global _VGGT_model, _VGGT_loaded
    if not _VGGT_loaded:
        print(f"Loading VGGT model from {model_path}...")
        _VGGT_model = VGGT.from_pretrained(model_path).to(device).eval()
        _VGGT_loaded = True
...
def extract_vggt_features_with_original_patches(
    ...
    # 获取已加载的模型（不会重复加载）
    model = load_vggt_model(model_path, device)
    
    # 不需要每次都 .to(device)，因为已经在设备上了
...
class RoboticDiffusionTransformerModel(object):
    ...
    def step(self, proprio, images, text_embeds):
        ...
        # 新加VGGT推理
        gemoetry_embeds = extract_vggt_features_with_original_patches(
