...
# 0523定义DGCA
class DualGateCrossAttention(nn.Module):
    def __init__(self, geo_dim=2048, sem_dim=1152):
        super(DualGateCrossAttention, self).__init__()
        self.geo_gate = nn.Linear(geo_dim, sem_dim)      # 2048 -> 1152
        self.sem_gate = nn.Linear(sem_dim, geo_dim)      # 1152 -> 2048
        # 注意：cross_attention 的 embed_dim 应该是 sem_dim (1152)
        self.cross_attention = nn.MultiheadAttention(embed_dim=sem_dim, num_heads=8, batch_first=True)

    def forward(self, geo_tokens, sem_tokens):
        # geo_tokens: [B, N, 2048] (Align feature)
        # sem_tokens: [B, N, 1152] (Siglip feature)
        
        # 门控
        geo_weight = torch.sigmoid(self.geo_gate(geo_tokens))  # [B, N, 1152]
        sem_weight = torch.sigmoid(self.sem_gate(sem_tokens))  # [B, N, 2048]
        
        # 互相门控
        F_sem_geo = sem_tokens * geo_weight   # [B, N, 1152]
        F_geo_sem = geo_tokens * sem_weight   # [B, N, 2048]
        
        # 残差
        F_sem_geo = F_sem_geo + sem_tokens    # [B, N, 1152]
        F_geo_sem = F_geo_sem + geo_tokens    # [B, N, 2048]
        
        # Cross-Attention: Q 来自 sem 分支 (1152维)
        # 需要把 F_geo_sem 投影到 1152 维作为 K, V
        F_geo_sem_proj = self.geo_gate(F_geo_sem)  # [B, N, 1152]  # 用 geo_gate 投影
        
        attn_output, _ = self.cross_attention(
            F_sem_geo,      # Q: [B, N, 1152]
            F_geo_sem_proj, # K: [B, N, 1152]
            F_geo_sem_proj  # V: [B, N, 1152]
        )
        
        return attn_output  # [B, N, 1152]
...
class RDTRunner(nn.Module,
    ...
    def __init__(self,
        ...
        # 0427新加VGGT+image token fusion adaptor
        self.concat_fusion_mlp = nn.Sequential(
            nn.Linear(3200, 2048),
            RmsNorm(2048,eps=1e-6),
            nn.SiLU(),
            nn.Linear(2048, 1152),
            RmsNorm(1152,eps=1e-6),
        )
        # 0523新加DGCA
        # self.dgca = DualGateCrossAttention(geo_dim=2048, sem_dim=1152)
    ...
    def adapt_conditions(self, lang_tokens, img_tokens, geo_tokens, state_tokens):
        '''
        lang_tokens: (batch_size, lang_len, lang_token_dim)
        img_tokens: (batch_size, img_len, img_token_dim)
        geo_tokens: (batch_size, geo_len, geo_token_dim)
        ...
        # 0427实现concat
        fusion_token = self.concat_fusion_mlp(torch.cat([img_tokens, geo_tokens], dim=-1))
        # fusion_token = self.dgca(geo_tokens, img_tokens)
        adpated_img = self.img_adaptor(fusion_token)
   ...
   # ========= Train  ============
    def compute_loss(self, lang_tokens, lang_attn_mask, img_tokens, geo_tokens,
        ...
        lang_cond, img_cond, state_action_traj = self.adapt_conditions(
            lang_tokens, img_tokens, geo_tokens, state_action_traj)
    ...
    # ========= Inference  ============
    def predict_action(self, lang_tokens, lang_attn_mask, img_tokens, geo_tokens, state_tokens,
        ...
        lang_cond, img_cond, state_traj = self.adapt_conditions(
            lang_tokens, img_tokens, geo_tokens, state_tokens)
